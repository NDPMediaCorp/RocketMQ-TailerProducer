...
public class ReliableTaildirEventReader implements ReliableEventReader {
  ...
  private ReentrantReadWriteLock filesLock = new ReentrantReadWriteLock();
    ...
    try {
      ...
      for (TaildirMatcher taildir : taildirCache) {
        ...
        for (File f : taildir.getMatchingFiles()) {
          ...
          TailFile tf = tailFiles.get(inode);
          if (tf == null) {
            //|| !tf.getPath().equals(f.getAbsolutePath())
            logger.info("inode {} not in tailFiles",inode);
  ...
  public long getInode(File file) throws IOException {
